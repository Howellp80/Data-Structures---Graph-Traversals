...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	Deque * dequeStack = dequeNew();

	// add source to stack
	dequePushFront(dequeStack, source);

	// while stack isnt empty
	while (!dequeIsEmpty(dequeStack)){

		// get the top vertex
		Vertex * top = dequeFront(dequeStack);
		dequePopFront(dequeStack);

		// if it matches the destination
		if (top == destination){
			dequeDelete(dequeStack);
			return 1;
		}
		// add its reachable neighbors (unvisited ones) to the stack
		else{
			top->isVisited = 1;
			for (int i = 0; i < top->numNeighbors; i++){
				Vertex * neighbor = top->neighbors[i];
				if (neighbor->isVisited == 0)
					dequePushFront(dequeStack, neighbor);
			}
		}
	}
	// if stack is empty,
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	Deque * dequeQueue = dequeNew();

	// add source to queue
	dequePushFront(dequeQueue, source);

	// while queue isnt empty
	while (!dequeIsEmpty(dequeQueue)){

		// get the top vertex
		Vertex * front = dequeFront(dequeQueue);
		dequePopFront(dequeQueue);

		// if it matches the destination
		if (front == destination){
			dequeDelete(dequeQueue);
			return 1;
		}
		// add its reachable neighbors (unvisited ones) to the queue
		else{
			front->isVisited = 1;
			for (int i = 0; i < front->numNeighbors; i++){
				Vertex * neighbor = front->neighbors[i];
				if (neighbor->isVisited == 0)
					dequePushBack(dequeQueue, neighbor);
			}
		}
	}
	// if queue is empty,
